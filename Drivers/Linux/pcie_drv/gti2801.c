...
static ssize_t show_gti_info(struct device *dev, struct device_attribute *attr, char *buf){
	...
	GTI_DEV * gti = (GTI_DEV *)pci_get_drvdata(pdev);
	struct fpga_dev * fpga=(struct fpga_dev *)gti->private;
...
static ssize_t gti_read(struct file *file, char __user *buf, size_t count,loff_t *pos)
{
	...
	gti->fpga->state=0;
	{
		...
		desc->bytes = count;
		desc->dst_addr_hi= cpu_to_le32(PCI_DMA_H(virt_to_bus(gti->buffer)));
		desc->dst_addr_lo= cpu_to_le32(PCI_DMA_L(virt_to_bus(gti->buffer)));
		gti_debug("dst addr:%08x;%08x\n",(desc->dst_addr_hi),(desc->dst_addr_lo));
		desc->src_addr_hi= 0;
		desc->src_addr_lo= (gti->channel * GYR_FIP_SPACING) + 0x80000;
...
static ssize_t gti_write(struct file *file, const char __user *buf,size_t count, loff_t *pos)
{
	...
	gti->fpga->state=0;
	{
		...
		desc->control= DESC_MAGIC | 0x13;
		desc->bytes = count;
		desc->src_addr_hi= cpu_to_le32(PCI_DMA_H(virt_to_bus(gti->buffer)));
		desc->src_addr_lo= cpu_to_le32(PCI_DMA_L(virt_to_bus(gti->buffer)));
		gti_debug("src addr:%08x;%08x\n",(desc->src_addr_hi),(desc->src_addr_lo));
		desc->dst_addr_hi= 0;
		desc->dst_addr_lo= (gti->channel * GYR_FIP_SPACING) + (gti->mode << 12);
		desc->next_hi=desc->next_lo=0;
...
int gti2801_init(void *gti_dev){
    ...
    if(rc==0) rc = request_threaded_irq(pdev->pDev->irq, gti_isr_th,0, 0, "GTI2801_drv", fpga);
	...
	if(rc==0){
		for(i=0;i<GTI_CHANNEL_NUMBER;i++){
			...
			printk(KERN_NOTICE "channel dma buffer:%px:%llx\n",fpga->gti[i].buffer,virt_to_bus(fpga->gti[i].buffer));
    ...
    if(rc==0){
		...
		w = cpu_to_le32(PCI_DMA_L(virt_to_bus(fpga->tx_descriptor)));
		...
		w = cpu_to_le32(PCI_DMA_L(virt_to_bus(fpga->rx_descriptor)));
